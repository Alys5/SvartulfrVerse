# Visual Baseline — Database Record

---

## Migration Metadata

| Field | Value |
|-------|-------|
| Source | old_template_and_source/visual/Visual_Baseline.md (decommissioned) |
| Authority | Visual Authority |
| Migration Date | 2026-06-08 |
| Status | Migrated |

---

## Purpose

This document defines the canonical visual baselines for the Los Angeles Dynasty. All visual characteristics flow from these baselines.

---

## Visual Baseline Definitions

### Douglas Visual Baseline

**Reference Character:** Erik Douglas

| Attribute | Value | Status |
|-----------|-------|--------|
| Hair Color | Black (streaked with silver/grey) | Canonical |
| Eye Color | Amber | Canonical |
| Build | Massive, muscular, imposing | Canonical |
| Height | 205cm | Canonical |
| Skin | Weathered, mature | Canonical |
| Aesthetic | Corporate monarch, power suits | Canonical |

**Visual DNA Anchor:**
```
Erik Douglas:
  Always:
    - Black hair streaked with silver/grey, slicked back
    - Amber eyes, cold and unyielding
    - 205cm, massive muscular build, weathered skin
    - Corporate monarch aesthetic, bespoke power suits
  Never:
    - Long white hair
    - Ancient king aesthetic
    - Nordic patriarch styling or fur robes
    - T-shirts or casual wear
```

---

### Bloodmoon Visual Baseline

**Reference Character:** Wulfnic Bloodmoon

| Attribute | Value | Status |
|-----------|-------|--------|
| Hair Color | Blonde | Canonical |
| Eye Color | Blue | Canonical |
| Build | Lean, strong, refined | Canonical |
| Height | 195cm | Canonical |
| Skin | Pale, refined | Canonical |
| Aesthetic | Ancestral nobility, old-world aristocratic | Canonical |

**Visual DNA Anchor:**
```
Wulfnic Bloodmoon:
  Always:
    - Blonde hair, long and braided with silver
    - Blue eyes, steady and knowing
    - 195cm, lean but impossibly strong, refined skin
    - Ancestral nobility aesthetic, heavy furs
  Never:
    - CEO aesthetic
    - Executive skyscrapers or corporate boardrooms
    - Modern streetwear or tactical gear
```

**Note:** Recovered canon establishes Bloodmoon visual DNA as blonde hair and blue eyes. Previous documentation referenced silver-white hair/eyes, which has been reconciled to the canonical baseline.

---

## Baseline Comparison

| Attribute | Douglas | Bloodmoon |
|-----------|---------|-----------|
| Hair Color | Black (silver with age) | Blonde |
| Eye Color | Amber | Blue |
| Build | Massive, muscular | Lean, refined |
| Height | 205cm | 195cm |
| Aesthetic | Corporate monarch | Ancestral nobility |
| Vibe | Modern power | Ancient authority |

---

## Inheritance Context

These baselines serve as the reference points for the Visual Fusion Model (see [W_Visual_Inheritance.md](./W_Visual_Inheritance.md)).

Children of Erik Douglas + Nixara Bloodmoon inherit visual characteristics according to fusion rules:
- **Douglas-visual-dominant:** Black hair, amber eyes, massive build
- **Bloodmoon-visual-dominant:** Blonde hair, blue eyes, lean build
- **Fusion-visual:** Blended characteristics (caramel-brown hair, mint-green eyes)

---

## Cross-References

- [W_Visual_Inheritance.md](./W_Visual_Inheritance.md) — Visual Fusion Model

---

## Authority

Established by: ADR-004
Record custodian: Visual Authority Layer
Last validated: 2026-06-07
Migrated: 2026-06-08
