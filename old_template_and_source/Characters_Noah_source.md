# Noah Douglas — Character Source Evidence

**Extraction Date:** 2026-06-07  
**Source Files:** C_Noah.md, C_Noah.js  
**Target:** database/characters/C_Noah.md (future migration)  
**Status:** Approved

---

## Accepted Evidence

### Family Role

| Element | Value | Notes |
|---------|-------|-------|
| Role | Middle son | Primary identity |
| Position | Second-born | Birth order |
| Generation | Douglas-Bloodmoon heirs | Lineage |

---

### Narrative Function

| Element | Value | Notes |
|---------|-------|-------|
| Archetype | The Velvet Glove | Primary archetype |
| Secondary | Legal Authority | Professional role |
| Dynamic | Social shield | Family function |

---

### Key Relationships

| Relation | Entity | Type |
|----------|--------|------|
| Father | Erik Douglas | Biological |
| Mother | Nixara Bloodmoon | Deceased |
| Brothers | Malachia, Jasper | Biological |
| Sister | Alyssa | Biological |

---

### Stable Character Traits

| Trait | Value |
|-------|-------|
| Core | Elegant |
| Core | Diplomatic |
| Core | Legally precise |
| Core | Socially manipulative |
| Core | Image-obsessed |
| Core | Emotionally distant |

---

### Family Authority Position

| Element | Value |
|---------|-------|
| Role | Legal & Social Liaison |
| Function | PR damage control |
| Position | Family lawyer |
| Secondary | Reputation management |

---

## Academic Timeline Resolution

**Authority:** LA_OnlyHuman_Academic_Timeline

**Decision:** Migration Drift

### Approved Current Status

| Field | Value | Notes |
|-------|-------|-------|
| Status | 3L Law Student | Per approved age-career progression |
| Education | Law School | Current trajectory |

### Approved Career Trajectory (Future)

| Field | Value | Notes |
|-------|-------|-------|
| Future | Corporate Lawyer | Post-graduation |
| Future | Family Legal Counsel | Future role |

### Rejected Current Status

| Field | Value | Reason |
|-------|-------|--------|
| Rejected | Corporate Lawyer | Violates approved age-career progression |
| Rejected | PR Director | Violates approved age-career progression |
| Rejected | Senior Executive | Violates approved age-career progression |
| Rejected | Legal Executive | Violates approved age-career progression |

**Reason:** These represent historical variants that violate the approved age-career progression model.

---

## Family Authority Validation

Validated against approved Family Authority record.

Genealogical authority is external.

Character records must not redefine:

- parentage
- lineage
- inheritance order
- dynasty membership

All genealogical information originates from Family Authority.

---

## Deferred Evidence

| Element | Status | Reason |
|---------|--------|--------|
| Harvard Law education | Deferred | Requires verification |
| Baking hobby | Deferred | Requires character review |
| Biometric watch | Deferred | Requires technology review |

---

## Rejected Evidence

### Professional Drift

| Element | Reason |
|--------|--------|
| Corporate Lawyer (current) | Migration drift — violates age-career progression |
| PR Director (current) | Migration drift — violates age-career progression |
| Legal Executive (current) | Migration drift — violates age-career progression |

---

### Runtime Systems

| System | Reason |
|--------|--------|
| All JavaScript code | Platform-specific implementation |
| Micro-engine code | Runtime behavior |
| Trigger systems | Janitor-specific |

---

### Visual Generation Pack

| Element | Reason |
|--------|--------|
| Steps/CFG/Sampler | Deployment settings |
| Negative Prompt | Deployment artifact |
| All image prompts | Not repository material |

---

### NSFW Content

| Element | Reason |
|--------|--------|
| Intimacy fields | Not Character Authority domain |
| Kinks/behavior | Not repository material |

---

### Stat Blocks

| Element | Reason |
|--------|--------|
| Social Battery | Runtime metric |
| Risk Approach | Runtime metric |
| Morality/Trust | Runtime metric |

---

### Multi-World Content

| World | Reason |
|-------|--------|
| Pack world | Supernatural variant — reject |
| Cyber world | Sci-fi variant — reject |
| Jarn world | Fantasy variant — reject |
| Werewolf species | Supernatural — reject |
| Delta rank | Supernatural hierarchy — reject |

---

## Migration Notes

**Recommended Baseline:**

```
Noah = Legal Authority Figure
```

Current status: 3L Law Student (approved)

Future trajectory: Corporate Lawyer, Family Legal Counsel

**No runtime code preserved.**

---

## Authority

**Extraction Type:** Character Source Evidence  
**Date:** 2026-06-07  
**Status:** Approved — Academic timeline resolved, family role preserved
