# Wulfnic Bloodmoon — Character Source Evidence

**Extraction Date:** 2026-06-07  
**Source Files:** C_Wulfnic.md, C_Wulfnic.js  
**Target:** database/characters/C_Wulfnic.md (future migration)  
**Status:** Approved

---

## Accepted Evidence

### Family Role

| Element | Value | Notes |
|---------|-------|-------|
| Role | Patriarch | Primary identity |
| Position | Previous-generation authority | Family position |
| Generation | Bloodmoon Dynasty | Lineage |

---

### Narrative Function

| Element | Value | Notes |
|---------|-------|-------|
| Archetype | The Ancient Sovereign | Primary archetype |
| Secondary | The Patriarch | Authority role |
| Dynamic | Spiritual guide | Family function |

---

### Key Relationships

| Relation | Entity | Type |
|----------|--------|------|
| Daughter | Nixara Bloodmoon | Deceased |
| Son-in-law | Erik Douglas | By marriage |
| Grandchildren | Malachia, Noah, Jasper, Alyssa | Biological |

---

### Stable Character Traits

| Trait | Value |
|-------|-------|
| Core | Detached from modern world |
| Core | Ancient authority |
| Core | Spiritual grounding |
| Core | Protective of bloodline |
| Core | Speaks in riddles/metaphors |

---

### Family Authority Position

| Element | Value |
|---------|-------|
| Role | Genealogical authority node |
| Function | Custodian of family memory |
| Position | More architecture than character |

---

## Historical Origin Resolution

**Decision:** Rejected Canon

### Rejected Elements

| Element | Reason |
|--------|--------|
| 1200 BC origin | Superseded by approved Family Authority chronology |
| Thrace origin | Superseded by Iceland per ADR-001 |
| Immortal Founder | Human patriarch only per ADR-001 |
| Ancient Alpha | Supernatural hierarchy — reject |
| Mythological Bloodline Origin | Superseded by approved chronology |

### Approved Family Authority

```
Wulfnic Bloodmoon (1948)
        │
        ↓
Nixara Bloodmoon (1975-2005)
        │
        ↓
Erik Douglas (1970) + Nixara Bloodmoon (1975-2005)
        │
        ↓
Malachia (1996)
Noah (1999)
Jasper (2005)
Alyssa (2005)

Nixara dies during childbirth in 2005.
```

---

## Visual Authority Alignment

Validated against ADR-004.

| Field | Original | Corrected | Reason |
|-------|----------|-----------|--------|
| Eyes | Silver-white, glowing | Blue | Superseded by approved baseline |
| Hair | Pure white, long | Blonde → Silver-white (age) | Per ADR-004 |
| Aesthetic | Ancient Norse robes | Contemporary patriarch | Superseded |
| Mystical glow | Faint glow | None | Supernatural drift |

---

## Family Authority Validation

Validated against approved Family Authority record.

Genealogical authority is external.

Character records must not redefine:

- parentage
- lineage
- inheritance order
- dynasty membership

All genealogical information originates from Family Authority.

---

## Deferred Evidence

| Element | Status | Reason |
|---------|--------|--------|
| Runic tattoos | Deferred | Requires visual audit |
| Ancient lore | Deferred | Requires cultural canon review |

---

## Rejected Evidence

### Supernatural Claims

| Element | Reason |
|--------|--------|
| 1200 BC origin | Superseded by Iceland (1930s migration) per ADR-001 |
| Immortal timeline | Human patriarch only per ADR-001 |
| Werewolf species | Supernatural — reject |
| Alpha of Alphas | Supernatural hierarchy — reject |
| Seiðr-master | Magic — reject |
| Soul-weaving | Supernatural — reject |
| Shadow-walking | Supernatural — reject |

---

### Runtime Systems

| System | Reason |
|--------|--------|
| All JavaScript code | Platform-specific implementation |
| Micro-engine code | Runtime behavior |
| Trigger systems | Janitor-specific |

---

### Visual Generation Pack

| Element | Reason |
|--------|--------|
| Steps/CFG/Sampler | Deployment settings |
| Negative Prompt | Deployment artifact |
| All image prompts | Not repository material |

---

### Multi-World Content

| World | Reason |
|-------|--------|
| Pack world | Supernatural variant — reject |
| Cyber world | Sci-fi variant — reject |
| Jarn world | Fantasy variant — reject |

---

## Migration Notes

**Recommended Baseline:**

```
Wulfnic = Patriarch Authority Node
```

More architecture than character.

**Critical Correction:** Wulfnic is NOT the founder of the Bloodmoon Dynasty. Per Wulfnic Founder Reconciliation Report, he is the Patriarch of the American Bloodmoon Branch and custodian of Svartúlfr heritage.

---

## Authority

**Extraction Type:** Character Source Evidence  
**Date:** 2026-06-07  
**Status:** Approved — Patriarch role preserved, supernatural claims rejected, historical origin resolved
